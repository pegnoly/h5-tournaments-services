...
use super::{
    payloads::{
        ChallongeData, ChallongeParticipantAttributes, ChallongeParticipantPayload,
        ChallongeParticipantsBulkAddPayload, ChallongeParticipantsBulkAttributes,
...
impl ChallongeClient {
    ...
    pub async fn post<T: Serialize>(
        ...
        api_key: &String,
        params: &str,
        payload: ChallongeData<T>,
    ) -> Result<reqwest::Response, reqwest::Error> {
        let client_locked = self.client.read().await;
        let response = client_locked
            .post(format!("{}{}", self.url, &params))
            .header("Accept", "application/json")
            .header("Content-Type", "application/vnd.api+json")
            .header("Authorization-Type", "v1")
            .header("Authorization", api_key)
...
impl ChallongeService {
    ...
    pub async fn get_tournaments(
        &self,
        api_key: &String,
    ) -> Result<Vec<ChallongeTournamentSimpleData>, crate::Error> {
        let response = self
            .client
            .get(api_key, "tournaments.json?page=1&per_page=25")
            .await;
        match response {
            Ok(success) => match success.json::<ChallongeTournamentsSimple>().await {
                Ok(tournaments) => Ok(tournaments.data),
                Err(error) => {
                    tracing::error!(
                        "Error deserializing tournaments data: {}",
                        error.to_string()
                    );
            ...
            Err(failure) => {
                tracing::error!(
                    "Failed to fetch all user's tournaments: {}",
                    failure.to_string()
                );
    ...
    pub async fn add_participant(
        ...
        match response {
            ...
            Err(failure) => {
                tracing::error!(
                    "Failed to send add participant request: {}",
                    failure.to_string()
                );
            }
        }
    ...
    pub async fn participants_bulk_add(
        ...
        let payload = ChallongeParticipantsBulkAddPayload {
            _type: super::payloads::ChallongePayloadType::Participants,
            attributes: Some(ChallongeParticipantsBulkAttributes { participants: data }),
        };
        let response = self
            .client
            .post(
    ...
    pub async fn create_challonge_participant(
        &self,
        api_key: &String,
        tournament_id: &String,
