...
use super::{commands::MAIN_URL, service::ParserService, types::{HrtaParser, Parse, UniverseParser}, utils::ParsingDataModel};
...
pub async fn process_tournament(
    ...
    let races = load_races(client).await.unwrap();
    let heroes = load_heroes(client, mod_type).await.unwrap();
    ...
    match mod_type {
        ...
        ModType::Hrta => {
            process_messages(service, &messages, HrtaParser {}, &data_model);
        }
    }
}

fn process_messages<P: Parse>(service: &ParserService, messages: &Vec<Message>, parser: P, data: &ParsingDataModel) {
    for message in messages {
        let match_structure = service.parse_match_structure(&message.content, &parser);
        let opponents_data = service.parse_opponents(&parser, &match_structure, &data);
        let games_data = service.parse_games(&parser, &match_structure, &data);
    }
}

async fn load_races(client: &RwLockReadGuard<'_, Client>) -> Result<Vec<Race>, ()> {
...
async fn load_heroes(client: &RwLockReadGuard<'_, Client>, mod_type: ModType) -> Result<Vec<Hero>, ()> {
    let heroes_response = client
        .get(format!("{}/heroes/{}", MAIN_URL, mod_type as i32))
        .send()
        .await;
